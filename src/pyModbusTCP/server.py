...
class ModbusServer:
    ...
    # A callable that returns a protocol instance.
    # It will be called once for each client connection.
    class ModbusService(asyncio.Protocol):

        ###
        # Each Modbus RTU frame
        # must be received in 1 Modbus TCP frame.
        # a fragmented TCP frame which arrives here, in more than 1 part
        # will be ignored.
        #
        # If we will support fragmented frames, we have to use
        # a buffered receiving logic and a DICT() to store the
        # ModbusTCP TransactionID and associate the corresponding data
        def __init__(self, ext_engine=None):
            self.transport = None
            self.client_adr = None
            self.client_port = 0
            self.engine = ext_engine
            #
            #self.Requests = dict()
        ...
        def connection_made(self, transport):
            ...
            (self.client_adr, self.client_port) = transport.get_extra_info('peername')
            logger.debug('Connection from {}:{}'.format(self.client_adr, self.client_port))
            #self.reqState = 0

        def connection_lost(self, reason):
            logger.debug("connection_lost: {} | {}".format(self.client_adr, reason))
        ...
        def data_received(self, data):
            ...
            try:
                ...
                sessionData = ModbusServer.SessionData()
                sessionData.client.address = self.client_adr
                sessionData.client.port = self.client_port

                #if (len(data) > 7) and (self.reqState == 0):
                if (len(data) > 7):
                    ### init session data for new request
                    #sessionData.new_request()
                    ### receive mbap from client
                    sessionData.request.mbap.raw = data[0:7]
                    #self.reqState = 1
                else:
                    logger.info("Modbus-paket is too short from {}:{}.".format(self.client_adr, self.client_port))
                #
                #if (self.reqState == 1) and (len(self.data) >= sessionData.request.mbap.length):
                if (len(data) >= (sessionData.request.mbap.length + 6)):
                    #self.reqState = 2
                    ### receive pdu from client
                    sessionData.request.pdu.raw = data[7:(sessionData.request.mbap.length + 6)]
                    ### update response MBAP fields with request data
                    sessionData.set_response_mbap()
                    ### pass the current session data to request engine
                    if not self.engine is None:
                        #logger.debug("Call ext_engine with {}:{}.".format(sessionData.request.mbap.unit_id, bytes.hex(sessionData.request.pdu.raw)))
                        self.engine(sessionData)
                    ### send the tx pdu with the last rx mbap (only length field change)
                    self._send_all(sessionData.response.raw)
                    #self.reqState = 0
                else:
                    logger.info("Malformed Modbus-paket from {}:{}.".format(self.client_adr, self.client_port))
    ...
    def start(self):
        ...
        if not self.is_run:
            # set class attribute
            add_family = socket.AF_INET6 if self.ipv6 else socket.AF_INET
            #
            self.loop = asyncio.get_event_loop()
            ### init server and set socket options
            #factory = ProtocolFactory(ModbusServer.ModbusService, self._engine() )
            coroSrver = self.loop.create_server(functools.partial(ModbusServer.ModbusService, self._engine), self.host, self.port, family=add_family, backlog=100, reuse_address=True, reuse_port=True, start_serving=True)
            self._service = self.loop.run_until_complete(coroSrver)
            #
            # Get the server address and port
            localAdr = self._service.sockets[0].getsockname()
            logger.info("Modbusserver is listen on: {}:{}".format(localAdr[0], localAdr[1]))
            ...
            #   => this will not work the Socket must be created prior
            #      to  asyncio.start_server  and must be passed as param: sock=
            for sock in self._service.sockets:
            ...
            ### Start Server
            ### Only if  start_serving=False
            #self._service.start_serving()
            logger.info("pyModbusTCP-Server is running.")
            #
            #try:
            #    logger.info("pyModbusTCP-Server is running.")
            ...
            #finally:
            #    self._service.close()
            #    self.loop.run_until_complete(self._service.wait_closed())
            #    self.loop.close()
            #    self._evt_running.clear()
        else:
            logger.info("Modbusserver is already running.")
    ...
    async def _serve(self):
        try:
            ...
            await self._service.server_close()
            raise
        except KeyboardInterrupt:
            await self._service.server_close()
        finally:
